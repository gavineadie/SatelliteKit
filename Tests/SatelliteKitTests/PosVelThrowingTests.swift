...
class PosVelThrowingTests: XCTestCase {
    ...
    func testSatPosition() {
        ...
        do {
            let _ = try sat.position(minsAfterEpoch: 1000_000_000.0)
            let _ = try sat.velocity(minsAfterEpoch: 1000_000_000.0)

            let _ = try sat.position(julianDays: 1000_000_000.0)
            let _ = try sat.velocity(julianDays: 1000_000_000.0)
        } catch {
            print(error)
        }
    }
