/*╔══════════════════════════════════════════════════════════════════════════════════════════════════╗
  ...
  ║ Created by Gavin Eadie on Jul06/15.
 Copyright 2015-20 Ramsay Consulting. All rights reserved. ║
  ║──────────────────────────────────────────────────────────────────────────────────────────────────║
  ╚══════════════════════════════════════════════════════════════════════════════════════════════════╝*/
...
/// Sidereal time is a system of timekeeping based on the rotation of the Earth with respect to the fixed stars in the sky. Specifically, it is the measure of the hour angle of the vernal equinox. When the measurements are made with respect to the meridian at Greenwich, the times are referred to as Greenwich mean sidereal time (GMST).
///
/// References: The 1992 Astronomical Almanac, page B6. http://celestrak.com/columns/v02n02/ and http://aa.usno.navy.mil/faq/docs/GAST.php
...
public func zeroMeanSiderealTime(julianDate: Double) -> Double {
    // fractional part of JD + half a day
    let     fractionalDay = fmod(julianDate + 0.5, 1.0)
    let     adjustedJD = julianDate - fractionalDay
    let     timespanCenturies = (adjustedJD - JD.epoch2000) / TimeConstants.daysPerCentury
    // Greenwich Mean Sidereal Time (secs)
    var     greenwichSiderealSeconds = 24110.54841 +
...
/// Solar Geo
/// - Parameter julianDays: julianDays
/// - Returns:  Declination (delta) and Right Ascension (alpha) are returned as decimal degrees.
...
/// Procedure eci2geo() will calculate the geodetic (lat, lon, alt) position of an object given the time and its ECI position. It is intended to be used to determine the satellite ground track. The calculations assume the earth to be oblate. If the time is negative, treat as zero.
/// - Parameters:
